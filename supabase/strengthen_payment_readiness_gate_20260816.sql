...
-- Keeps payment initiation behind one authoritative database gate.

alter table public.site_settings
  add column if not exists payment_quote_ttl_minutes integer not null default 30;

alter table public.site_settings
  drop constraint if exists site_settings_payment_quote_ttl_minutes_check;

alter table public.site_settings
  add constraint site_settings_payment_quote_ttl_minutes_check
  check (payment_quote_ttl_minutes between 5 and 180);
...
begin
  ...
  select coalesce(payment_quote_ttl_minutes, 30)
    into ttl_minutes
  ...
  if o.coupon_code is not null then
    ...
    if not found or c.active is not true then
      return jsonb_build_object('ok', false, 'reason', 'coupon_unavailable');
    end if;
    ...
    if c.usage_limit is not null and coalesce(c.used_count,0) >= c.usage_limit then
      return jsonb_build_object('ok', false, 'reason', 'coupon_limit_reached');
    end if;
  end if;
  ...
  loop
    if coalesce(item->>'fulfillmentReady','false') <> 'true' then
      return jsonb_build_object(
        'ok', false,
        'reason', 'order_snapshot_not_fulfillment_ready',
        'productId', item->>'id'
      );
    end if;
    ...
    if r.ready_for_paid_order is not true then
      return jsonb_build_object(
        'ok', false,
        'reason', 'current_product_not_ready',
        'productId', r.id,
        'blockers', to_jsonb(r.blockers)
      );
    end if;
    ...
    if r.supplier_product_id is distinct from (item->>'supplierProductId')
      ...
      return jsonb_build_object(
        'ok', false,
        'reason', 'supplier_mapping_changed',
        'productId', r.id
      );
    end if;
  end loop;
