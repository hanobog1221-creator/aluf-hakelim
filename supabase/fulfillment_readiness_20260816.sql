-- Strict supplier readiness view applied to production on 2026-08-16.
-- Unknown or stale supplier data is a blocker, not a successful state.
...
select
  ...
  (
    ...
    and (
      coalesce(p.minimum_profit, s.minimum_profit_ils) is null
      or p.selling_price - p.supplier_price_ils >= coalesce(p.minimum_profit, s.minimum_profit_ils)
    )
    and (
