...
test('sets a whole-shekel price that preserves at least 25 ILS before configured fees', () => {
  assert.equal(requiredSellingPrice(40.2, 25), 82);
});

test('pricing includes fee and reserve configuration while shipping is charged separately', () => {
  const pricing = pricingForOffer(offer(), 25, { paymentFeePercent: 4, paymentFeeFixedIls: 1, reserveIls: 2 });
  assert.equal(pricing.customerShipping, 12);
  assert.ok(pricing.projectedNetProfit >= 25);
...
test('keeps 25 ILS after the configured 40 percent tax and insurance reserve', () => {
  const pricing = pricingForOffer(offer(), 25, { taxReservePercent: 22, insuranceReservePercent: 18 });
  assert.ok(pricing.projectedNetProfit >= 25);
