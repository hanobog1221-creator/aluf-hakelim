...
module.exports = async function handler(req, res) {
  ...
  if (!tokenResponse.ok || !token || !token.access_token || (token.code && String(token.code) !== '0')) {
    console.error('AliExpress token exchange failed', tokenResponse.status, tokenText.slice(0, 500));
  ...
  if (!dbResponse.ok) {
    console.error('AliExpress token storage failed', dbResponse.status, (await dbResponse.text()).slice(0, 300));
