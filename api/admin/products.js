  ...
  try {
    ...
    if (req.method === 'PATCH') {
      ...
      if ('categories' in body) {
        if (!Array.isArray(body.categories) || body.categories.length > 10) return res.status(400).json({ ok: false, error: 'invalid_categories' });
        update.categories = body.categories.map((v) => String(v).trim()).filter(Boolean).slice(0, 10);
      }
      if ('specs' in body) {
        if (!Array.isArray(body.specs) || body.specs.length > 30) return res.status(400).json({ ok: false, error: 'invalid_specs' });
        update.specs = body.specs.map((v) => String(v).trim()).filter(Boolean).slice(0, 30);
      }
      ...
      return res.status(200).json({ ok: true, product: rows[0] || null });
    }
