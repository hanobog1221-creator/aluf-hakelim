...
async function writeHistory(product, row) {
  try {
    ...
    console.warn('AliExpress catalog history write failed:', error.message);
  }
}
...
async function updateProduct(product, snapshot) {
  ...
  const availableSkus = snapshot.skus.filter((sku) => sku.inStock !== false);
  const fallback = availableSkus.find((sku) => sku.price != null) || snapshot.skus.find((sku) => sku.price != null) || null;
  ...
  if (source?.price != null && source?.currency) supplierPriceIls = await convertToIls(source.price, source.currency);
  ...
  if (selected?.id) {
    ...
    shippingError = 'selected_sku_missing';
  }
  ...
  await writeHistory(product, {
    ...
    error: update.supplier_sync_error || update.shipping_sync_error || null
  });
  return { ready, update };
}
...
async function markError(product, error) {
  ...
  await fetch(`${supabaseUrl}/rest/v1/products?id=eq.${encodeURIComponent(product.id)}`, {
    method: 'PATCH',
    headers: serverHeaders({ 'Content-Type': 'application/json', Prefer: 'return=minimal' }),
    body: JSON.stringify({ supplier_sync_error: message, fulfillment_ready: false, updated_at: new Date().toISOString() })
  }).catch(() => {});
  await writeHistory(product, { error: message });
  return message;
}

async function syncAllActiveAliExpressProducts() {
  const { supabaseUrl } = serverConfig();
  const response = await fetch(`${supabaseUrl}/rest/v1/products?select=*&active=eq.true&supplier=eq.aliexpress&supplier_product_id=not.is.null&order=sort_order.asc&limit=50`, {
