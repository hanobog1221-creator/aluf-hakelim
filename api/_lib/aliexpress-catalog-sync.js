...
function normalizedSku(sku) {
  ...
  return { id: idRaw == null ? null : String(idRaw), attr: skuAttributePath(sku), label: skuLabel(sku), inStock, stock: Number.isFinite(stockCount) ? stockCount : null, price: Number.isFinite(price) ? price : null, currency: sku?.currency_code || sku?.currencyCode || null, shape: compactShape(sku) };
}
...
async function callProduct(productId) {
  try { return await callProductOnce(productId); }
    ...
    if (!/ApiCallLimit/i.test(code)) throw error;
    await sleep(2500);
    return callProductOnce(productId);
  }
}
...
async function updateProduct(product, snapshot) {
  ...
  if (selected?.id && /^\d{5,30}$/.test(selected.id)) {
    try { freight = await quoteAliExpressFreight({ productId:product.supplier_product_id||snapshot.productId, skuId:selected.id, qty:1, countryCode:'IL', shipFromCountry:product.supplier_ship_from_country||'CN' }); }
    catch (error) { shippingError = String(error.code || error.message || error).slice(0,300); }
  } else shippingError = selected ? 'selected_numeric_sku_id_missing' : 'selected_sku_missing';
  const now = new Date().toISOString(); const skuVerified = Boolean(selected); const skuAttrVerified = Boolean(selected?.attr); const shippingAvailable = Boolean(freight);
...
async function markError(product,error) {
  const { supabaseUrl }=serverConfig(); const message=String(error.code||error.message||error).slice(0,300);
  await fetch(`${supabaseUrl}/rest/v1/products?id=eq.${encodeURIComponent(product.id)}`,{method:'PATCH',headers:serverHeaders({'Content-Type':'application/json',Prefer:'return=minimal'}),body:JSON.stringify({supplier_sync_error:message,fulfillment_ready:false,updated_at:new Date().toISOString()})}).catch(()=>{});
  await writeHistory(product,{error:message}); return message;
}
async function syncAllActiveAliExpressProducts() {
  const { supabaseUrl }=serverConfig(); const response=await fetch(`${supabaseUrl}/rest/v1/products?select=*&active=eq.true&supplier=eq.aliexpress&supplier_product_id=not.is.null&order=sort_order.asc&limit=50`,{headers:serverHeaders()});
  if(!response.ok) throw new Error(`products_read_${response.status}`); const products=await response.json(); const results=[];
  for (let index=0; index<products.length; index+=1) {
    const product=products[index]; if(index>0) await sleep(900);
    try { const snapshot=await callProduct(product.supplier_product_id); const saved=await updateProduct(product,snapshot); results.push({id:product.id,ok:true,ready:saved.ready,inStock:saved.update.supplier_in_stock,shippingAvailable:saved.update.supplier_shipping_available,skuAttrReady:Boolean(saved.update.supplier_sku_attr)}); }
    catch(error){ const message=await markError(product,error); results.push({id:product.id,ok:false,error:message}); }
  }
  return { synced:results.length, succeeded:results.filter((row)=>row.ok).length, results };
}
