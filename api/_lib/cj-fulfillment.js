...
function buildRequests(order, supplierState, sandbox) {
  ...
  return items.map((item,index) => {
    ...
    const request = {
      orderNumber: orderNumber(order.order_id,index),
      shippingZip: clean(customer.postalCode || '0000000',20),
      shippingCountry: 'Israel', shippingCountryCode:'IL',
      shippingProvince: clean(customer.city || 'Israel',100), shippingCity: clean(customer.city,100),
      shippingPhone: clean(customer.phone,40), shippingCustomerName: clean(customer.fullName,100),
      shippingAddress: clean(customer.street,160), houseNumber: clean(customer.houseNumber,30),
      email: clean(customer.email || 'orders@aluf-hakelim.invalid',160),
      remark: clean(customer.notes || `Aluf Hakelim ${order.order_id}`,300),
      payType: 3,
      isSandbox: sandbox ? 1 : 0,
      logisticName: clean(shipping.serviceName,120),
      ...
      products:[{
        vid:String(current.fulfillment_variant_id), sku:String(current.fulfillment_sku), quantity:Number(item.qty),
        storeProductId:String(item.id), storeProductName:clean(item.name || current.name,200), storeLineItemId:`${order.order_id}-${index+1}`
      }]
    };
...
function fingerprint(requests) {
  return crypto.createHash('sha256').update(JSON.stringify(requests.map(x=>x.request)),'utf8').digest('hex');
}
...
async function cjCreate(request) {
  ...
  const r = await fetch(`${CJ_BASE}/shopping/order/createOrderV2`, {
    method:'POST', headers:{Accept:'application/json','Content-Type':'application/json','CJ-Access-Token':token}, body:JSON.stringify(request)
  });
  const raw=await r.text(); let json={}; try{json=raw?JSON.parse(raw):{}}catch{json={message:raw.slice(0,300)}}
  const code=Number(json?.code);
  if(!r.ok||json?.result===false||json?.success===false||(Number.isFinite(code)&&![0,200].includes(code))){
    const e=new Error(`cj_order_${json?.code||r.status}_${clean(json?.message||'failed',180)}`);e.cj=json;throw e;
  }
  const data=json.data||{}; const id=clean(data.orderId||data.id,80);
  if(!id) throw new Error('cj_order_id_missing');
  return { id, orderNumber:clean(data.orderNumber||request.orderNumber,80), data, requestId:json.requestId||null };
}

async function preflightCjOrder(orderId) {
  const { order, validation, supplierState } = await getFulfillmentCandidate(orderId);
  if (!validation.ok) return { ok:false, validation };
  const sandbox=sandboxMode();
  const requests=buildRequests(order,supplierState,sandbox);
  return { ok:true, order, validation, sandbox, requests, requestFingerprint:fingerprint(requests) };
}

async function fulfillCjOrder(orderId) {
  const preflight=await preflightCjOrder(orderId);
  ...
  const attempt=await dbInsert('supplier_order_attempts',{
    ...
    response:{requests:preflight.requests.map(x=>({itemId:x.itemId,orderNumber:x.request.orderNumber,sandbox:preflight.sandbox}))}
  });
  ...
  const providerCost=created.reduce((sum,x)=>sum+Number(x.data?.orderAmount||0),0) || null;
  await dbPatch('supplier_order_attempts',`id=eq.${attempt.id}`,{
    status:'payment_pending', supplier_order_ids:ids, provider_order_number:created[0]?.orderNumber||null,
    provider_payment_required:true, provider_payment_completed:false, provider_cost:providerCost, provider_currency:'USD',
    response:{created:created.map(x=>({id:x.id,orderNumber:x.orderNumber,itemId:x.itemId,postageAmount:x.data?.postageAmount??null,productAmount:x.data?.productAmount??null,logisticsMiss:x.data?.logisticsMiss??null}))}
  });
  await dbPatch('orders',`order_id=eq.${encodeURIComponent(orderId)}`,{
    supplier_order_id:ids[0]||null, supplier_order_ids:ids, fulfillment_status:'ordering', last_error:null
  });
