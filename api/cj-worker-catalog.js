...
function chooseFreight(rows,preferred){const valid=(Array.isArray(rows)?rows:[]).filter(x=>Number.isFinite(Number(x?.logisticPrice))&&Number(x.logisticPrice)>=0);if(preferred){const hit=valid.find(x=>clean(x?.logisticName,120).toLowerCase()===clean(preferred,120).toLowerCase());if(hit)return hit}valid.sort((a,b)=>Number(a.logisticPrice)-Number(b.logisticPrice));return valid[0]||null}
...
async function getFreight(vid,origin){const j=await cj('/logistic/freightCalculate',{method:'POST',body:{startCountryCode:origin,endCountryCode:'IL',products:[{quantity:1,vid:String(vid)}]}});return Array.isArray(j.data)?j.data:[]}
async function syncProduct(p,minProfit,job=null){
  const vid=clean(p.fulfillment_variant_id||job?.provider_variant_id,160),sku=clean(p.fulfillment_sku||job?.provider_variant_sku,180),pid=clean(p.fulfillment_product_id||job?.provider_product_id,180);if(!vid||!sku||!pid)throw new Error('cj_identity_incomplete');
  const detail=await getVariant(vid);if(!detail)throw new Error('cj_variant_missing');const origin=originCountry(detail),stock=inventoryTotal(detail),freight=await getFreight(vid,origin),selected=chooseFreight(freight,p.fulfillment_logistic_name||job?.provider_snapshot?.selectedFreight?.logisticName);if(!selected)throw new Error('cj_shipping_to_il_unavailable');
  const priceUsd=Number(detail.variantSellPrice),freightUsd=Number(selected.logisticPrice);if(!Number.isFinite(priceUsd)||priceUsd<0)throw new Error('cj_price_missing');
  const priceIls=await convertToIls(priceUsd,'USD'),shippingIls=await convertToIls(freightUsd,'USD'),target=priceIls+Number(minProfit||25),sellingPrice=retailAtLeast(target,p.selling_price),now=new Date().toISOString(),totalCost=Number((priceIls+shippingIls).toFixed(2));
...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const settings=(await dbGet('site_settings?id=eq.primary&select=minimum_profit_ils&limit=1'))[0]||{},minProfit=Number(settings.minimum_profit_ils||25),products=await dbGet('products?select=*&active=eq.true'),byId=new Map(products.map(p=>[String(p.id),p])),jobs=await dbGet('product_intake_jobs?select=*&provider_status=eq.cj_quote_ready'),results=[],synced=new Set();
